...
csv_file = open('VOCDevTest.csv', 'w')
#Insert the header
...
for image_set in sets:
    image_ids = open('Datasets/VOC2007_Test/ImageSets/Main/%s.txt' % (image_set)).read().strip().split()
    for image_id in image_ids:
        
        #Open up the annotation file
        ann_file = open('Datasets/VOC2007_Test/Annotations/%s.xml' % (image_id))
